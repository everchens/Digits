...
def parse_args():
    ...
    parser.add_argument('--p_pad',
                        help='Ratio to randomly pad an image',
                        type=float, default=1.0)
    parser.add_argument('--pad_size',
                        help='The ratio of padded image size to original image size, in area',
                        type=float, default=10.0)
    parser.add_argument('--pad_hw_vari',
                        help='Variation of h/w ratio',
                        type=float, default=0.1)
...
def augment_images(filelist, args):
    for filepath, n in filelist:
        ...
        for i in range(n):
            ...
            if random.random() < args.p_pad:
                img_varied = ia.random_pad(
                    img_varied,
                    args.pad_size,
                    args.pad_hw_vari)
                varied_imgname += 'p'
